...
(async function(){
  ...
  function renderAdminServers(){
    ...
    if(!SERVER_RATES.length){ root.innerHTML = '<p class="admin-topup-empty">ยังไม่มีเซิร์ฟเวอร์ในระบบ</p>'; return; }
    root.innerHTML = SERVER_RATES.map(function(s, idx){ return adminServerRowHtml(s, idx, SERVER_RATES.length); }).join('');
  }
  document.getElementById('adminServerAddBtn').addEventListener('click', function(){
    ...
    if(SERVER_RATES.some(function(s){ return s.id===id; })){ errEl.textContent = 'รหัสนี้มีอยู่แล้ว ตั้งรหัสใหม่'; return; }
    var nextSort = SERVER_RATES.reduce(function(m,s){ return Math.max(m, s.sortOrder||0); }, 0) + 1;
    btn.disabled = true;
  ...
  adminServerListEl.addEventListener('drop', function(e){
    ...
    var fromIdx = SERVER_RATES.findIndex(function(s){ return s.id===draggedServerId; });
    var toIdx = SERVER_RATES.findIndex(function(s){ return s.id===targetId; });
    if(fromIdx===-1 || toIdx===-1) return;
    var reordered = SERVER_RATES.slice();
    var moved = reordered.splice(fromIdx, 1)[0];
    reordered.splice(toIdx, 0, moved);
    SERVER_RATES = reordered;
    renderAdminServers();
  ...
  document.getElementById('adminServerList').addEventListener('click', function(e){
    ...
    if(delBtn){
      ...
      var srv = SERVER_RATES.filter(function(s){ return s.id===did; })[0];
